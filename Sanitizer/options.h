...
namespace gwp_asan
{
	namespace options 
	{
		...
		typedef void(*Printf_t)(const wchar_t*Format, ...);
		...
		// GuardedPoolAllocator contains a reentrant barrier to prevent infinite
		...
		// e.g. "  #5 0x420459 in _start (/tmp/Raf+0x420459)"
		...
		struct Options 
		{
			...
			int MaxSimultaneousAllocations = 12800; // 默认包含12800个内存分配
			int SampleRate = 250;
			bool Enabled = true;
			bool InstallExceptionHandlers = true;
			...
			void setDefaults() 
			{
				...
				PerfectlyRightAlign = false;
				MaxSimultaneousAllocations = 256000;
				SampleRate = 1;
				Enabled = true;
				InstallExceptionHandlers = true;
			}
		};
	} // namespace options
} // namespace gwp_asan
